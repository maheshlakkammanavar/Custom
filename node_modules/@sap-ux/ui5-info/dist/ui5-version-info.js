 ...
 */
async function parseUI5Versions(url = constants_1.ui5VersionRequestInfo.OfficialUrl.toString()) {
    const response = await requestUI5Versions(url);
    let result = [];
    if (Array.isArray(response.routes)) {
        result = response.routes.map((route) => {
            if (route.path === '/') {
...
async function parseUI5VersionsOverview() {
    ...
    let versions = [];
    try {
        const response = await requestUI5Versions(constants_1.ui5VersionRequestInfo.OfficialUrl, `/${constants_1.ui5VersionRequestInfo.VersionsOverview}`);
        ...
        versions = ui5_version_fallback_1.ui5VersionFallbacks;
    }
    result = versions.map((ver) => {
        const parsedVersion = (0, semver_1.coerce)(ver.version)?.version;
...
const retrieveUI5VersionsCache = async (type, useCache = false, snapshotUrl) => {
    if (!useCache) {
        switch (type) {
            case "officialVersions" /* ui5VersionsType.official */:
                return parseUI5Versions(constants_1.ui5VersionRequestInfo.OfficialUrl);
            case "snapshotsVersions" /* ui5VersionsType.snapshot */:
                if (snapshotUrl) {
                    return parseUI5Versions(snapshotUrl);
                }
                break;
            case "overview" /* ui5VersionsType.overview */:
    ...
    if (constants_1.ui5VersionsCache[type].length === 0) {
        switch (type) {
            case "officialVersions" /* ui5VersionsType.official */:
                constants_1.ui5VersionsCache[type] = await parseUI5Versions(constants_1.ui5VersionRequestInfo.OfficialUrl);
                break;
            case "snapshotsVersions" /* ui5VersionsType.snapshot */:
                if (snapshotUrl) {
                    constants_1.ui5VersionsCache[type] = await parseUI5Versions(snapshotUrl);
                }
                break;
            case "overview" /* ui5VersionsType.overview */:
                constants_1.ui5VersionsCache[type] = await parseUI5VersionsOverview();
                break;
            default:
        }
    }
...
async function getUI5Versions(filterOptions) {
    ...
    if (filterOptions?.includeMaintained) {
        ui5VersionsOverview = (await retrieveUI5VersionsCache("overview" /* ui5VersionsType.overview */, filterOptions.useCache));
    }
